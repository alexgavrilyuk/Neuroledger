...
class AgentExecutor {
     ...
     * @property {Array<{type: string, content: string}>} fragments - Array to store { type: 'text' | 'step', ... } for saving
     */
    ...
    constructor(userId, teamId, sessionId, aiMessagePlaceholderId, sendEventCallback, initialPreviousAnalysisData = null, initialPreviousGeneratedCode = null) {
        ...
        this.turnContext = {
            ...
            intermediateResults: {
                ...
                analysisResult: initialPreviousAnalysisData, // Result from execute_analysis_code
                generatedReportCode: initialPreviousGeneratedCode,
                fragments: [], // <<< ADDED: Array to store { type: 'text' | 'step', ... } for saving
            },
            userContext: '',
            teamContext: '',
    ...
    async runAgentLoopWithStreaming(userMessage, sessionDatasetIds = []) {
        ...
        try {
            ...
            while (iterations < MAX_AGENT_ITERATIONS) {
                ...
                // Reset per-LLM-call state
                currentLLMResponse = ''; 
                parsedToolCallFromStream = null;
                finalAnswerFromLLM = null; 
                llmFinished = false;

                // 1. Prepare context (including latest step results)
                const llmContext = this._prepareLLMContextForStream();

                // 2. Call LLM (Streaming)
                const streamResult = await streamLLMReasoningResponse(
                    llmContext,
                    (eventType, data) => {
                        // Callback for handling events DURING the stream
                        // This callback primarily updates the *frontend* via SSE
                        // We no longer accumulate the full response here for backend parsing.
                        switch (eventType) {
                            case 'token':
                                // Forward token for frontend streaming display
                                this._sendStreamEvent('token', data);
                                break;
                            case 'tool_call': 
                                // Potentially parse tool call *during* the stream (e.g., Claude)
                                // For now, just log it. Parsing happens on complete response.
                                logger.debug(`[Agent Stream CB] Received tool_call event: ${data.toolName}`);
                                // We might parse this early if needed: parsedToolCallFromStream = { tool: data.toolName, args: data.input };
                                break;
                             case 'finish':
                                logger.debug(`[Agent Stream CB] Received finish event. Reason: ${data.finishReason}`);
                                llmFinished = true;
                                // If finish reason indicates a tool call was intended (like Claude), we might parse early
                                break;
                            case 'completed':
                                logger.debug('[Agent Stream CB] Received completed signal from prompt service.');
                                llmFinished = true; // Mark as finished
                                break;
                             case 'error':
                                logger.error(`[Agent Stream CB] Received error signal from prompt service: ${data.message}`);
                        ...
                        if (eventType === 'token' && data.content) {
                            ...
                            if (lastFragment && lastFragment.type === 'text') {
                                lastFragment.content += data.content; // Merge consecutive text
                            } else {
                        ...
                        // Need to handle other events like 'finish', 'completed', 'error' as before
                            ...
                            logger.error(`[Agent Stream CB] Received error signal from prompt service: ${data.message}`);
                            throw new Error(data.message);
                        } else if (eventType !== 'token') { // Avoid logging every token
                             logger.debug(`[Agent Stream CB] Received event type: ${eventType}`);
                        }
                        // --- END ADDED --- 
                    }
                );

                // --- ADDED: Use the full response returned by streamLLMReasoningResponse --- 
                const fullLLMResponseText = streamResult; 
                if (fullLLMResponseText === null) { // Handle case where stream itself had an error
                    throw new Error(this.turnContext.error || "LLM streaming failed to return a response.");
                }
                logger.debug(`[AgentExecutor ${this.sessionId}] Full raw response from LLM stream (length ${fullLLMResponseText?.length}): "${fullLLMResponseText?.substring(0, 500)}..."`);
                // --- END ADDED --- 

                // 3. Determine Action (based on full response, stream events)
                let action = null;
                let isFinalAnswer = false;
                
                // Now parse the FULL response text returned by the streaming function
                if (llmFinished) {
                    logger.debug(`[AgentExecutor ${this.sessionId}] Attempting to parse full LLM response text.`);
                    // Use the returned full text for parsing
                    const parseResult = this._parseCompleteLLMResponse(fullLLMResponseText);
                    ...
                    // Store potential final answer text derived from the FULL response
                    this.turnContext.finalAnswer = parseResult.textResponse;
                     } else {
                    // Stream likely ended due to an error caught by the callback
                    logger.warn(`[AgentExecutor ${this.sessionId}] LLM stream ended without a finish/completed signal. Relying on error context.`);
                    // Error should have been thrown by the callback, but handle defensively
                    action = { tool: '_answerUserTool', args: { textResponse: this.turnContext.error || "An error occurred during processing." } };
                    isFinalAnswer = true;
                    this.turnContext.finalAnswer = action.args.textResponse;
                }

                // --- ADDED: If it's a final answer, add it as the last text fragment --- 
                ...
                if (action.tool === '_answerUserTool' || isFinalAnswer) {
                    ...
                    if (!this.turnContext.finalAnswer) {
                        this.turnContext.finalAnswer = action.args?.textResponse || fullLLMResponseText.trim() || "I apologize, but I couldn't formulate a response.";
                    ...
                    // --- ADDED: Include generated code AND analysis data in the final answer event --- 
                    this._emitAgentStatus('agent:final_answer', { 
                        text: this.turnContext.finalAnswer,
                        aiGeneratedCode: this.turnContext.intermediateResults.generatedReportCode, // Include code if available
                        analysisResult: this.turnContext.intermediateResults.analysisResult // <<< ADD analysis result
                    });
                    // --- END ADDED --- 
            ...
            await this._updatePromptHistoryRecord(
                ...
                this.turnContext.intermediateResults.fragments // <<< Pass fragments
            );
            ...
            try {
                await this._updatePromptHistoryRecord('error', null, errorMessage, null, null, this.turnContext.intermediateResults.fragments);
            } catch (updateError) {
    ...
    _storeIntermediateResult(toolName, toolResult) {
         ...
         try {
            ...
            // --- ADDED: Store completed step as a fragment --- 
            if (toolName !== '_answerUserTool' && toolName !== '_maxIterations' && toolName !== '_unknown') {
                const stepFragment = {
                    type: 'step',
                    tool: toolName,
                    resultSummary: summarizeToolResult(toolResult), // Use the summary
                    error: toolResult.error || null,
                    status: toolResult.error ? 'error' : 'completed'
                };
                this.turnContext.intermediateResults.fragments.push(stepFragment);
                logger.debug(`[AgentExecutor ${this.sessionId}] Stored step fragment: ${toolName}`);
            }
            // --- END ADDED --- 
         } catch (storageError) {
     ...
     * @param {Array<{type: string, content: string}>} fragments - Array of text fragments received from the LLM stream.
     */
    async _updatePromptHistoryRecord(status, aiResponseText = null, errorMessage = null, aiGeneratedCode = null, analysisData = null, fragments = []) {
        ...
        try {
            const updateData = {
                ...
                messageFragments: fragments, // <<< Save fragments
            };
