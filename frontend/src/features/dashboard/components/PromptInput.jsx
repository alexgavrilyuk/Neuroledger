...
const PromptInput = ({
    ...
    // Group datasets by team vs. personal
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-3 transition-all duration-300">
            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden ${expanded ? 'max-h-80' : 'max-h-0 opacity-0 mb-0'}`}>
                <div className="p-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-gradient-subtle-light dark:bg-gradient-subtle-dark shadow-soft-sm dark:shadow-soft-dark-sm">
                    ...
                    {datasetsLoading ? (
                        ...
                        <div className="max-h-48 overflow-y-auto custom-scrollbar pr-1">
                            {/* Personal Datasets */}
                            {groupedDatasets.personalDatasets.length > 0 && (
                                <div className="mb-3">
                                    <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">
                                        Personal Datasets
                                    </h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                                        {groupedDatasets.personalDatasets.map((ds) => (
                                            <div
                                                key={ds._id}
                                                className={`
                            ...
                            {/* Team Datasets */}
                            {Object.entries(groupedDatasets.teamDatasets).map(([teamName, teamDatasetsArray]) => (
